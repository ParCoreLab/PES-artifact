...
static void our_handler(int signum, siginfo_t *info, void *uc) {
	...
	int tid = syscall(SYS_gettid);
        ret=ioctl(fd[tid], PERF_EVENT_IOC_DISABLE, 0);


        count_total++;

        ret=ioctl(fd[tid], PERF_EVENT_IOC_ENABLE,1);
...
#pragma omp parallel
	...
	fd[tid]=perf_event_open(&pe,0,-1,-1,0);
	if (fd[tid]<0) {
		fprintf(stderr,"Problem opening leader %s\n",
			strerror(errno));
			//test_fail(test_string);
	}
	our_mmap[tid]=mmap(NULL, mmap_pages*4096,
		PROT_READ|PROT_WRITE, MAP_SHARED, fd[tid], 0);
	...
	for (i = 0; i < SIZE; i += 32) {
        ...
        ret=ioctl(fd[tid], PERF_EVENT_IOC_ENABLE,0);
	...
	{
	start = clock();
        ioctl(fd[tid], PERF_EVENT_IOC_PEBS_SAMPLE_COUNT, 0);	
	}
	#pragma omp barrier
        //#pragma omp parallel
        //{
	__asm__ __volatile__ ("movl $1000000, %%edx\n\t"
		...
		#pragma omp master
		{
			pebs_count = ioctl(fd[tid], PERF_EVENT_IOC_PEBS_SAMPLE_COUNT, 0);
        		ret=ioctl(fd[tid], PERF_EVENT_IOC_REFRESH,0);
			end = clock();
		}
		free(indices);
		munmap(our_mmap[tid],mmap_pages*4096);
		close(fd[tid]);
